...
#############
my_named_list %>% 
  .$thing_1 # access via dollarsign
my_named_list %>% 
  .[[1]] # access via dollarsign
```
...
corr %>% 
  str()
# drilling down with dollar sign notation
corr %>% 
  .$estimate %>% 
  .[[1]]
...
`r apply_reveal("statistical_function_returns")`
...
`r apply_reveal("compare")`



```{r}
list(apples = c("mac", "granny"), 
     oranges = c("naval", "blood")) %>% 
  .$oranges
```
