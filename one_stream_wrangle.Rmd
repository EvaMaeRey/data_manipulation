...
# .large.white[one-stream <br>wrangle]
### .white[An exploration <br>with {flipbookr}<br>and {xaringan}]
...
#### .right.white[Gina Reynolds<br>Photo Credit: Vlad Tchompalov]


```{r, include = F}

knitr::opts_chunk$set(cache = T, comment = "", warning = F)
```
  ...
  mutate(
    pop_category =
      case_when(pop >= 1000000000 ~ "billions",
                ...
                pop <  100000000 ~ 
                  "fewer than hundred million")) %>% 
  mutate(
    pop_category = 
      fct_relevel(pop_category, #BREAK2
